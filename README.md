...
- **Animation ลื่นไหล:** ขับเคลื่อนด้วย **Framer Motion** (LazyMotion) สำหรับเอฟเฟกต์เปิดตัว การเลื่อนหน้าจอ และลูกเล่นต่างๆ
- **Parallax Scrolling:** ประสบการณ์หน้า Landing Page ที่น่าตื่นตาตื่นใจ พร้อมระบบ Scroll-snap (ปรับแต่งพิเศษสำหรับมือถือ)
...
- **Modular Code:** แยก Logic การคำนวณ Animation ออกเป็น Custom Hook (`useLuckyDraw`) เพื่อความเสถียร
...
- **Smart Fetching:** ใช้ `AbortController` และ Custom Hooks (`usePreregisterStatus`) เพื่อจัดการการดึงข้อมูลอย่างมีประสิทธิภาพและป้องกัน Memory Leak
...
- **Canonical URL:** ระบุ URL หลักเพื่อป้องกันปัญหา Duplicate Content
...
- `hooks/`: Custom React Hooks (`useLuckyDraw`, `usePreregisterStatus`)
- `lib/`: ฟังก์ชัน Utility และไฟล์ตั้งค่า (`preregister-config.js`, `db.js`)
- `public/`: ไฟล์ Static (รูปภาพ, ไอคอน)
- `__tests__/`: ไฟล์ Unit Tests
