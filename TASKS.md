...
- [ ] **Fix Rate Limiting Logic (BUG-002)**
    ...
    - [ ] **Action:** เปลี่ยนจากเช็ค IP Address เป็น `session.user.id` (Discord ID)
    - [ ] **File:** `app/api/user/register/route.js`
    - [ ] **Action:** เปลี่ยนจากเช็ค IP Address เป็น `session.user.id`
- [ ] **Remove Dead Code & Double Commit (BUG-003)**
    - [ ] **File:** `app/api/luckydraw/route.js`
    - [ ] **Action:** ลบโค้ดส่วนเกินหลัง `return` และ `connection.commit()` ที่ซ้ำซ้อน

## 🟡 Priority 2: UX/UI Improvements (ประสบการณ์ผู้ใช้)
> *แก้ไขเพื่อให้ใช้งานง่ายและรองรับมือถือได้ดีขึ้น*

- [ ] **Fix Touch Targets (Mobile)**
    - [ ] **Action:** ขยายขนาดปุ่ม "Spin" และปุ่มเมนูให้มีความสูงอย่างน้อย 44px
- [ ] **Add Loading States**
    - [ ] **Page:** Register
    - [ ] **Action:** เพิ่ม Loading Spinner หรือ Disable ปุ่มเมื่อกด "ลงทะเบียน"
    - [ ] **Page:** Lucky Draw
    - [ ] **Action:** เพิ่ม Feedback "กำลังหมุน..." ระหว่างรอ API
- [ ] **Improve User Feedback**
    - [ ] **Feature:** Daily Check-in
    - [ ] **Action:** เพิ่ม Toast/Notification แจ้งเตือนเมื่อกดรับรางวัล (Success/Error)
- [ ] **Fix Layout Shift (CLS)**
    - [ ] **Page:** News
    - [ ] **Action:** กำหนด `width` และ `height` หรือ `aspect-ratio` ให้กับรูปภาพข่าว
    - [ ] **Global:** ตรวจสอบการโหลด Font (FOUT) อาจใช้ `next/font` เพื่อ Optimize
